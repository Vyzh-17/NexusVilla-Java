...
public class SecurityGUI extends JFrame {
    private final SecurityService svc = new SecurityService();
    private final int securityId;
    private final int communityId;
    private String communityName;

    public SecurityGUI(int securityId, int communityId) {
    ...
    private void initializeFrame() {
        setTitle("NexusVilla - Security Panel");
        setSize(1300, 800);
    ...
    private JPanel createHeader() {
        ...
        JLabel title = new JLabel("Security Panel");
        ...
        JLabel securityLabel = new JLabel("Security Officer");
        securityLabel.setFont(new Font("Segoe UI", Font.PLAIN, 14));
        securityLabel.setForeground(new Color(107, 114, 128));
        
        JLabel communityLabel = new JLabel(communityName);
        communityLabel.setFont(new Font("Segoe UI", Font.BOLD, 12));
        communityLabel.setForeground(Color.WHITE);
        communityLabel.setBackground(new Color(245, 158, 11));
        communityLabel.setOpaque(true);
        communityLabel.setBorder(BorderFactory.createCompoundBorder(
            BorderFactory.createLineBorder(new Color(217, 119, 6), 1),
            BorderFactory.createEmptyBorder(6, 12, 6, 12)
        ));

        userInfo.add(securityLabel);
        userInfo.add(communityLabel);
    ...
    private JPanel createSidebar() {
        ...
        JLabel userInfo = new JLabel("<html><b style='color:#1e293b;font-size:12px;'>Security Officer</b><br>" +
                "<span style='color:#64748b;font-size:11px;'>" + communityName + "</span></html>");
        ...
        // Navigation
        String[] menuItems = {
            "Verify Visitor (OTP)", "View Visitor Log", "Logout"
        };
    ...
    private JPanel createWelcomeSection() {
        ...
        JLabel welcome = new JLabel("Security Dashboard");
        welcome.setFont(new Font("Segoe UI", Font.BOLD, 28));
        welcome.setForeground(new Color(31, 41, 55));

        JLabel subtitle = new JLabel("Monitor and verify visitor access for " + communityName);
    ...
    private JPanel createStatsGrid() {
        ...
        grid.add(createStatCard("Today's Visitors", "18", new Color(16, 185, 129), "👥"));
        grid.add(createStatCard("Pending Verifications", "3", new Color(245, 158, 11), "⏳"));
        grid.add(createStatCard("Verified Today", "15", new Color(59, 130, 246), "✅"));
        grid.add(createStatCard("Active Security", "2", new Color(139, 92, 246), "🛡️"));
    ...
    private JPanel createQuickActions() {
        ...
        JPanel actionsGrid = new JPanel(new GridLayout(1, 2, 20, 20));
        actionsGrid.setBackground(new Color(249, 250, 251));

        actionsGrid.add(createActionCard("Verify Visitor", "", "✅"));
        actionsGrid.add(createActionCard("View Visitor Log", "", "📋"));
    ...
    private void handleNavigation(String text) {
        switch (text) {
            case "Verify Visitor (OTP)":
                showVerifyVisitorDialog();
                break;
            case "View Visitor Log":
                new ViewVisitorLogFrame("security", 0);
                break;
            case "Logout":
                dispose();
                new LoginFrame();
    ...
    private void showVerifyVisitorDialog() {
        JTextField house = new JTextField(15);
        JTextField otp = new JTextField(15);
        ...
        int result = JOptionPane.showConfirmDialog(this, panel, "Verify Visitor", 
            JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);

        if (result == JOptionPane.OK_OPTION) {
            svc.verifyVisitor(house.getText().trim(), otp.getText().trim());
        }
    }
